...
public class CommandsManager {
	...
	public void process(Message msg, Member author) {
		...
		case "total":
			...
			msg("**TOTAL A PAYER AUX EMPLOYES**", 
				"  " + total + "$");
			break;
		...
		case "inactive":
			...
			for(PlayerData dat : inst.saver.getAllData()) {
				if(!dat.stillActive()) {
					txta += dat.displayName + " -> " + dat.toPay() + "$\n";
				}
			}
			msg(txta + "```");
			break;
		...
		case "tableau":
			String txt = "```\n"
					+ ""
					+ "|        NOM        |Reste à payer|Déjà $ (Semaine)|\n";
			for(PlayerData dat : inst.saver.getAllData()) {
				String line = "| " + dat.displayName;
				while(line.length() < 20) line += " ";
				line += "| " + dat.toPay() + "$";
				while(line.length() < 34) line += " ";
				line += "| " + dat.paidThisWeek() + "$";
		...
		case "pay":
			...
			if(amount > 70_000) {
				...
				+ 	"Les primes ne peuvent pas être plus élevé que 70k.");
				return;
			}
			
			if(amount % PlayersContainer.RUN_PAY != 0) {
				msg(":x: **Montant pas rond...** :x:\n"
				+ 	"Vous devez payer un multiple de " + PlayersContainer.RUN_PAY);
				return;
			}
			
			try {
				data.pay(amount / 2000);
